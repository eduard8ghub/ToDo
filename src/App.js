...
function App() {
    ...
    const [list, setLists] = useState(DB.lists.map(item => {
        item.color = DB.colors.filter(color => item.id === color.id)[0].name;
        return item;
    }));

    const getNewList = (newTextList, newColorId) => {
        const lengthList = list.length + 1;
        const color = DB.colors.filter(color => color.id === newColorId)[0].name;
        const newList = {
            "id": lengthList,
            "name": newTextList,
            "colorId": newColorId,
